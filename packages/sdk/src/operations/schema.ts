...
export async function createObject(
  ...
  if (await loadObject(workspaceDatabase(workspace), input.object_slug)) {
    throw new AcrmError(
...
export async function addAttribute(
  ...
  await assertObjectExists(workspaceDatabase(workspace), input.object_slug);
  if (input.config && input.config.target_object) {
    await assertObjectExists(workspaceDatabase(workspace), input.config.target_object as string);
  }
  if (await loadAttribute(workspaceDatabase(workspace), input.object_slug, input.attribute_slug)) {
    throw new AcrmError(
